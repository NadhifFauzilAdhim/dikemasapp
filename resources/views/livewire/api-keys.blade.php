<div x-data="{ tab: 'keys' }">
    ...
    <div class="mb-6 border-b border-slate-200 dark:border-slate-800">
        <nav class="-mb-px flex space-x-8">
            <button @click="tab = 'keys'" 
                    :class="tab === 'keys' ? 'border-amber-500 text-amber-600 dark:text-amber-500' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300'"
                    class="whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium transition-colors">
                API Keys Management
            </button>
            <button @click="tab = 'docs'" 
                    :class="tab === 'docs' ? 'border-amber-500 text-amber-600 dark:text-amber-500' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300'"
                    class="whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium transition-colors">
                API Documentation
            </button>
        </nav>
    ...
    <div x-show="tab === 'keys'" style="display: none;" x-transition.opacity>
    ...
    <div x-show="tab === 'docs'" style="display: none;" x-transition.opacity>
        <div class="space-y-8">
            {{-- Violations API --}}
            <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <h3 class="mb-2 text-xl font-bold text-slate-900 dark:text-white">1. PPE Violations API</h3>
                <p class="mb-4 text-sm text-slate-500 dark:text-slate-400">Endpoint untuk mengirim data pelanggaran K3 (PPE) beserta bukti gambar.</p>
                
                <div class="mb-4 flex items-center gap-2">
                    <span class="rounded bg-indigo-100 px-2.5 py-1 text-xs font-bold text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-400">POST</span>
                    <code class="text-sm font-mono text-slate-700 dark:text-slate-300">/api/v1/violations</code>
                </div>
                ...
                <div class="mb-6">
                    <p class="mb-2 text-sm font-semibold">Body Parameters (FormData):</p>
                    <table class="w-full text-left text-sm text-slate-600 dark:text-slate-300">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            <tr><td class="py-2 pr-4 font-mono">camera_id</td><td class="py-2 pr-4">string</td><td class="py-2">Identifier kamera (ex: CAM-001)</td></tr>
                            <tr><td class="py-2 pr-4 font-mono">violation_type</td><td class="py-2 pr-4">string</td><td class="py-2">Tipe pelanggaran (ex: NO-Hardhat)</td></tr>
                            <tr><td class="py-2 pr-4 font-mono">confidence</td><td class="py-2 pr-4">numeric</td><td class="py-2">Tingkat akurasi AI (0.0 - 1.0)</td></tr>
                            <tr><td class="py-2 pr-4 font-mono">image</td><td class="py-2 pr-4">file</td><td class="py-2">File gambar (jpg/png) bukti pelanggaran</td></tr>
                            <tr><td class="py-2 pr-4 font-mono">bbox</td><td class="py-2 pr-4">json/array</td><td class="py-2">Koordinat kotak (x1, y1, x2, y2)</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
            ...
            {{-- Counting API --}}
            <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                <h3 class="mb-2 text-xl font-bold text-slate-900 dark:text-white">2. Production Monitoring API</h3>
                <p class="mb-4 text-sm text-slate-500 dark:text-slate-400">Endpoint untuk mengirim detak (*heartbeat*) dan menghitung jumlah produk yang lewat per interval.</p>
                
                <div class="mb-4 flex items-center gap-2">
                    <span class="rounded bg-indigo-100 px-2.5 py-1 text-xs font-bold text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-400">POST</span>
                    <code class="text-sm font-mono text-slate-700 dark:text-slate-300">/api/v1/counting/heartbeat</code>
                </div>
                ...
                <div class="mb-6">
                    <p class="mb-2 text-sm font-semibold">Body Parameters (JSON):</p>
                    <table class="w-full text-left text-sm text-slate-600 dark:text-slate-300">
                        <thead class="border-b border-slate-200 dark:border-slate-800">
                            <tr>
                                <th class="py-2 pr-4 font-semibold">Field</th>
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                            <tr><td class="py-2 pr-4 font-mono">camera_id</td><td class="py-2 pr-4">string</td><td class="py-2">Identifier kamera (ex: PROD-CAM-01)</td></tr>
                            <tr><td class="py-2 pr-4 font-mono">counts</td><td class="py-2 pr-4">integer</td><td class="py-2">Jumlah penambahan produk sejak sinkronisasi terakhir</td></tr>
                        </tbody>
                    </table>
                </div>
            </div>
